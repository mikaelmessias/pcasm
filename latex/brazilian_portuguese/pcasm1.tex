...
\subsection{Binário\index{binário|(}}
...
\subsection{Memória\index{memória|(}}
...
\subsection{A família de CPUs 80x86\index{CPU!80x86}}
...
\subsection{Interrupções\index{interrupções}}
...
\subsection{Linguagem de máquina\index{linguagem de máquina}}
...
\index{opcode}. As instruções do processador 80x86 variam em tamanho. O
...
\emph{montador} ou \emph{assembler} em inglês \index{montador} pode fazer
...
de máquina. A palavra {\code add} é chamado de {mnemônico} \index{mnemônico}
para a instrução deadição. A forma geral de uma instrução em assembly é:
\begin{CodeQuote}
  {\em mnemônico operando(s)}
\end{CodeQuote}

Um \emph{montador} \index{montador} é um programa que lê um arquivo de texto
com instruções assembly e converte a código de máquina. \emph{Compiladores}
\index{compiladores} são programas que fazem conversão similar de linguagens
...
Outra importante diferença entre assembly e linguagens de alto nível é que
cada tipo diferente de CPU tem sua própria linguagem de máquina, e também
tem sua própria linguagem de montagem (assembly). Portar programas assembly
entre diferentes arquiteturas de computadores é \emph{muito} mais difícil que
uma linguagem de alto nível.

Os exemplos desse livro utilizam o /Netwide Assembler/ ou, abreviando: NASM
\index{NASM}. É um programa livre, disponibilizado na Internet (veja o prefácio
para a encontrar a URL). Os outros montadores mais comuns são o montador da
Microsoft (MASM) \index{MASM} ou o montador da Borland (TASM). \index{TASM} Há
algumas diferenças na sintaxe do assembly para o MASM/TASM e o NASM.

\subsection{Operadores de instrução}

Instruções de código de máquina variam em número e tipo de operandos; entretanto,
em geral, cada instrução terá um fixo número de operandos (0 a 3). Operandos
podem ter os seguintes tipos:

\begin{description}
\item[register:]
  Esses operandos se referem diretamente ao conteúdo dos registradores da CPU.
\item[memory:]
  Esses se referem aos dados em memória. O endereço dos dados podem ser
  constantes programadas diretamente no código da instrução ou computadas
  usando valores dos registradores.
  Endereços são sempre deslocamentos do início de um segmento.
\item[immediate:]
  \index{immediate}
  Esses são valores fixos que são listados na própria instrução. Eles são
  armazenados na própria instrução (no segmento de código), não no segmento
  de dados.
\item[implied]
  Esses operandos não são explicitamente mostrados. Por exemplo, a instrução
  de incremento adiciona um ao reistrador ou à memória. O valor de incremento
  é implicado.
\end{description}
...
\subsection{Entrada e Saída \index{I/O|(}}
...
\subsection{Depuração\index{depuração|(}}
...
\subsubsection{Representações Big e Little Endian \index{endianess|(}}
